...
1. Discover markets via Kalshi API. **Check the bet journal first:** if this match was already analyzed (bet placed or pass logged in `~/.hermes/logs/bet_journal.jsonl`), check whether the current `yes_ask` has moved ≥3¢ from the price at that decision. If it hasn't moved ≥3¢, skip re-research entirely — the prior analysis stands. The free Kalshi price check gates the expensive paid X/Grok research; a 1-2¢ move is Kalshi spread noise, not new information.
...
**Read every cross-check BOTH ways.** Each signal below can discount a side OR bump it — a winless streak, a squad-value gap, or an injury crisis on the *underdog* makes the FAVORITE underpriced, not just the underdog overpriced. The engine has historically only applied the discount direction; apply the bump direction too (see ## Backing a favorite).
...
D. **Climate/Heat** — cold-climate team playing hot-climate opponent in an open-air hot venue. See `references/wc2026-venue-heat.md` (stadium data) and `references/heat-analysis-workflow.md` (step-by-step). Core signal: Scandinavian/British/northern European teams in Miami/Monterrey/Guadalajara afternoon kicks against African/Middle Eastern/Latin American opponents. When this signal fires, adjust fair value up to 5-7 cents against the cold team. The 2025 Club World Cup previewed the same problem — players reported dizziness, "impossible" conditions. The current **forecast** temperature may be pulled via `web_search` (Perplexity), but the venue/city/roof IDENTITY stays anchored to the reference per the substrate rule below — Perplexity NEVER names the venue.
   ...
   **Pre-computed calibration check (cost-saver):** `wc2026-venue-heat.md` contains worked examples for specific matches (e.g. SWE-TUN Jun 14 — see its "Worked Calibration Example" section). Before running a full Elo+FV+cross-check cycle on a match, scan the venue-heat reference for a pre-computed calibration — if one exists, verify the current Kalshi price hasn't moved enough to change the conclusion, then reuse the calibration. The SWE-TUN example saved an entire x_search + Elo interpolation cycle (Jun 14 cron).
...
## Backing a favorite (favorite-value plays, added Jun 14 2026)

Every match-win conviction to date has been the underdog or the cheap side (entry FV 0.25–0.37); **zero** favorite bets have ever been *proposed*, and **zero** have ever been *rejected* by the rails (journal-confirmed). So the skew is a **search blind spot, not a rails constraint** — Kairos hunts only reasons a favorite is *weak* and never reasons it's *underpriced*. Fix the search, not the code.

Most of the skew is correct: heavy favorites are the most efficiently priced contracts (highest volume, most attention), so the model rarely disagrees enough to clear the edge floor, and when it disagrees on a lopsided game the gap usually points to the cheap side. **"This team will win" is NOT an edge — the market already knows.** Germany over Curaçao at 92¢ is priced right; paying 92¢ to make 8¢ on a 92%-to-win side is roughly break-even after fees, not value. A favorite is winning on the pitch ≠ a favorite was a good bet.

A favorite is a BET only when the market is **too cautious** — a specific, sourced reason the price sits below true FV, exactly like any other edge. The favorite is then the *ideal* conviction bet: it rides to resolution, it usually wins (R7 trivially passes), and it's a low-variance way to grind the bankroll up — the opposite of the "too defensive" complaint. Hunt these as actively as underdog value. Triggers (market priced too low on a strong side):

- **Rotation/rest overreaction** — an already-qualified favorite rests starters for a dead rubber and the market overcorrects, but the B-team still beats a minnow. *Mirror caution:* do NOT back a rotated favorite against a *live* opponent that needs the result.
- **Recency-bias sag** — favorite lost/drew a friendly or a tough qualifier and the market sours, but Elo/squad-value fundamentals are unchanged → price too low.
- **Retail longshot bias** — retail overpays for longshots (the lottery payout), which can leave the heavy favorite a hair *cheap*. The mirror of why underdogs are often overpriced.
- **Doubt-then-confirmed stars** — the market prices in injury doubt on a key player; late confirmation they START removes the discount. Buy *before* the market reprices — the cleanest CLV-positive favorite play (dovetails with the step-6 X-signal scan).
- **Underdog worse than priced** — the mirror of the injury-crisis signal (item F): when the *underdog* is missing its spine (CB/CM/ST out, best player gone), bump the FAVORITE's FV, don't only discount the dog.

**Discipline (so this never becomes "pay full freight for chalk"):** the favorite still needs **net edge ≥ 5¢** (R2 high-FV floor) — that floor is the brake against picking up pennies in front of a steamroller (a 90¢ favorite that loses 10% of the time wipes ~9 wins), and it is NOT loosened. If the only reason to back the favorite is "they're better," there's no edge — PASS. Sizing is automatic and already generous here: confidence-scaled Kelly divides by (1−price), so a cleared favorite sizes near the 25%-of-capital cap — one good favorite bet is already a meaningful position; never force it bigger.
...
- **Kalshi `with_nested_markets=true` returns null/unreliable fields — including `status` and `subtitle`**: When fetching events with nested markets, `status`, `subtitle`, `yes_bid`, `yes_ask`, `volume`, `last_price`, AND `liquidity_dollars` are all unreliable from the nested objects. Status/subtitle render as `?` or `None`; prices are `null`; `liquidity_dollars` often shows `"0.0000"` even when the orderbook API has substantial depth. **Filtering by `e.get('status')=='open'` fails silently — all events appear rejected.** For event metadata (status, subtitle, kickoff time), fetch WITHOUT `with_nested_markets=true` (or accept that these fields will be null and filter by ticker pattern instead). For price data, always fetch individually via `GET /markets/{ticker}`. For liquidity, always use the orderbook API (`GET /markets/{ticker}/orderbook`). See `references/kalshi-api-pagination.md` for the full pagination pattern.
...
- **Market `status` + `result` are the liveness/settlement gates — read them PER MARKET; NEVER infer liveness or outcome from a price, and NEVER generalize one market to the whole slate.** A game day has matches at different kickoffs: early ones may already be settled while later ones are still `active`. Fetch each game's three markets individually (`GET /markets/{ticker}` — `status`/`*_dollars` reliable; `with_nested_markets` returns them null). **Kalshi never emits "placeholder"/"fake" prices** — a 1.00/0.00 quote is either a settled outcome or an indicative empty pre-match book; `status` tells you which, so never call a real exchange quote a placeholder.
  - `status=='active'` → live: EVALUATE it and (pre-match) observe the `-TIE`. An empty book (`yes_bid 0.00 / yes_ask 1.00`, no resting size) is an INDICATIVE pre-match book worth pricing, but a limit order **RESTS until a counterparty arrives** (placement ≠ a filled position; no market-maker backstop) — bound your price at fair value, never cross toward 0.99 just to force a fill, and count only filled quantity. An active 3-way book summing ≈1.00 (e.g. KSA 0.41 / URU 0.24 / draw 0.36) IS executable.
  - `status != 'active'` → NOT currently bettable. **Settlement truth is the `result` field, NOT last_price.** The game DREW iff the **`-TIE` ticker's `result=='yes'`** — a favorite that WON also finalizes its own ticker near 0.99, so never read "≈0.99 ⇒ draw" off a price; record win/draw/loss from which of the 3 mutually-exclusive tickers resolved YES. A non-active market with **no `result` yet** (pre-open / exchange-paused / awaiting settlement) is "result pending" — NOT dead, NOT a placeholder: re-poll, do not journal a result, and do not let it make you call the slate dead.
  - **"No bettable markets today" is only true AFTER you've individually priced every `active` game and journaled a real bet-or-pass decision (prices + edge reasoning) for each.** Concluding the whole day is dead from one finished/early game is the **Jun 15 bug**: Spain-CapeVerde and Belgium-Egypt had already DREW (settled), but Saudi-Uruguay and Iran-NZ were `active` with live prices and were wrongly reported "all dead" — so no bets AND no draw observations happened all day. Kickoff = event `sub_title` (e.g. "(Jun 15)") + market `expected_expiration_time` (~2h post-kickoff); drive liveness off `status`, not off `close_time` (a far-future default until settlement).
